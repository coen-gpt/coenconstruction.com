...
export default function EstimatePanel({ projectId, project }) {
  ...
  const { data: estimates = [] } = useQuery({
    ...
    onSuccess: (data) => {
      const orig = data.find((e) => e.type === "original" && e.status !== "superseded");
      if (orig && !localItems) {
        setLocalItems(orig.line_items || []);
        setLocalNotes(orig.notes || "");
        setLocalMarkup(orig.default_markup_pct || 20);
      }
    },
  });
  ...
  const items = localItems ?? originalEstimate?.line_items ?? [];
  ...
  const removeItem = (id) => setLocalItems((prev) => prev.filter((i) => i.id !== id));
  ...
  const saveEstimate = async () => {
    ...
    await base44.entities.ContractorProject.update(projectId, { original_estimate_total: grandTotal, adjusted_total: grandTotal });
  ...
  const generateAI = async () => {
    ...
    try {
      ...
      if (response.data?.line_items) {
        setLocalItems(response.data.line_items.map((i) => ({ ...i, id: i.id || crypto.randomUUID(), total: calcTotal(i) })));
    ...
    <div className="space-y-4">
      ...
      <div className="flex flex-wrap gap-2 items-center justify-between bg-white border border-gray-200 rounded-xl p-4">
        <div className="flex gap-2">
          <Button onClick={generateAI} disabled={generating} className="gap-2 bg-primary text-white text-sm">
            <Sparkles className="w-4 h-4" /> {generating ? "Generating..." : "AI Generate"}
          </Button>
          <Button variant="outline" onClick={() => addItem()} className="gap-1 text-sm">
            <Plus className="w-4 h-4" /> Add Line Item
          </Button>
        </div>
        <div className="flex gap-2 items-center">
          <label className="text-xs text-gray-500">Default Markup %</label>
          <Input type="number" value={localMarkup} onChange={(e) => setLocalMarkup(Number(e.target.value))} className="w-16 h-8 text-sm text-center" />
          <Button variant="outline" onClick={exportXLSX} className="gap-1 text-sm">
            <Download className="w-4 h-4" /> Export
          </Button>
          <Button onClick={saveEstimate} disabled={saving} className="gap-1 bg-secondary text-white text-sm">
      ...
      <div className="bg-secondary text-white rounded-xl px-5 py-3 flex justify-between items-center">
        ...
        <span className="text-2xl font-bold text-primary">${grandTotal.toLocaleString(undefined, { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</span>
      </div>
                ...
                <div key={item.id} className="p-4 grid gap-3">
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
                    ...
                    <div>
                      ...
                      <Input value={item.subgroup} onChange={(e) => updateItem(item.id, "subgroup", e.target.value)} className="h-8 text-sm" placeholder="Subgroup" />
                    </div>
                    <div>
                  ...
                  <div className="grid grid-cols-3 sm:grid-cols-6 gap-3 items-end">
                    ...
                    <div className="flex items-end">
                      <Button variant="ghost" size="icon" onClick={() => removeItem(item.id)} className="h-8 w-8 text-red-400 hover:text-red-600">
                  ...
                  <div>
                    ...
                    <Input value={item.internal_notes} onChange={(e) => updateItem(item.id, "internal_notes", e.target.value)} className="h-8 text-sm" placeholder="Internal notes (not shown to client)" />
                  </div>
                </div>
              ))}
      ...
      <div className="bg-white border border-gray-200 rounded-xl p-4">
        ...
        <Textarea rows={3} value={localNotes} onChange={(e) => setLocalNotes(e.target.value)} className="resize-none text-sm" placeholder="Terms, conditions, payment schedule..." />
      </div>
          ...
          <div className="space-y-2">
            {changeOrders.map((co) => (
              <div key={co.id} className="flex items-center justify-between p-3 bg-muted rounded-lg text-sm">
                <span>{co.title || `CO #${co.change_order_number}`}</span>
                <span className={`px-2 py-0.5 rounded-full text-xs font-semibold ${co.status === "approved" ? "bg-green-100 text-green-700" : "bg-yellow-100 text-yellow-700"}`}>
                  {co.status}
                </span>
