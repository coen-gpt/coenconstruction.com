...
export default function DesignPreviewCTA({ variant = "banner", page = "" }) {
  if (variant === "inline") {
    return (
      <div className="bg-gradient-to-r from-secondary to-secondary/90 rounded-xl p-6 text-white flex flex-col md:flex-row items-center gap-4">
        ...
        <Link to="/start" onClick={() => WebsiteEvents.designPreviewCTAClicked('inline', page)} className="shrink-0 bg-primary text-white font-bold px-6 py-3 rounded flex items-center gap-2 hover:bg-primary/90 transition-colors whitespace-nowrap">
  ...
  if (variant === "card") {
    return (
      <div className="bg-white border-2 border-primary/20 rounded-xl p-6 text-center shadow-lg">
        ...
        <Link to="/start" onClick={() => WebsiteEvents.designPreviewCTAClicked('card', page)} className="block bg-primary text-white font-bold py-3 rounded hover:bg-primary/90 transition-colors">
          Start Free Preview →
        </Link>
      </div>
    );
    ...
    <section className="bg-gradient-to-br from-secondary via-secondary to-secondary/95 py-16 px-4">
      <div className="max-w-4xl mx-auto text-center">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center">
          <Link to="/start" onClick={() => WebsiteEvents.designPreviewCTAClicked('banner', page)} className="bg-primary text-white font-bold px-10 py-4 rounded text-lg hover:bg-primary/90 transition-colors flex items-center gap-2 justify-center">
            <Sparkles className="w-5 h-5" /> Try Free Design Preview
          </Link>
          <a href="tel:6178572636" onClick={() => WebsiteEvents.phoneClicked(page || 'design_preview_banner')} className="border-2 border-white text-white font-bold px-8 py-4 rounded text-lg hover:bg-white/10 transition-colors">
            Call (617) 857-COEN
          </a>
        </div>
      </div>
